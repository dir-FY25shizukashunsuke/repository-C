...
│   │   └── update_submodule.yml # サブモジュール更新ワークフロー
│   └── skills/                  # AI スキル定義
│       └── update README/
...
サブモジュール `repository-A` には、**Flask (Python)** と **Express (Node.js)** の2つの実装が含まれています。
...
| `PATCH` | `/api/users/<user_id>` | ユーザー情報更新 | `{ "username", "email" }` (任意) | 200: `{ "message", "user" }` / 400: エラー / 404: Not Found |
...
| `GET` | `/` | ホーム（API 確認用） | - | `{ "message": "ユーザー登録API へようこそ！" }` |
| `POST` | `/api/users/register` | ユーザー登録 | `{ "username", "email", "password", "passwordConfirm" }` | 201: `{ "message", "user" }` / 400: エラー |
| `PATCH` | `/api/users/:id` | ユーザー情報更新 | `{ "username", "email" }` (任意) | 200: `{ "message" }` / 400: エラー / 404: Not Found |

### セキュリティ機能
- **パスワードハッシュ化**
  - Flask: `werkzeug.security.generate_password_hash`
  - Express: `bcryptjs` (bcrypt)
- **メールアドレス形式検証**（正規表現）
- **ユーザー名・メール重複チェック**
- **パスワード最小長**（6文字以上）
- **SQL インジェクション対策**
...
Flask と Express の両実装で共通のデータベーススキーマを使用しています。
...
- **[repository-A/README.md](./repository-A/README.md)**: サブモジュール (Flask/Express API) の詳細
...
**最終更新日時**: 2026-02-09 05:24:53 (UTC)
