...
sequenceDiagram
    ...
    API-->>Client: 200 OK
    Note over API,Client: { users: [...] }
```

### 3. ユーザー情報更新フロー（PATCH /api/users/<user_id>）
...
sequenceDiagram
    ...
    Client->>FastAPI: POST /upper
    Note over Client,FastAPI: { "text": "hello" }
    
    FastAPI->>API: to_upper(text)
    API->>Logic: text.upper() 実行
    Logic-->>API: "HELLO"
    API-->>FastAPI: "HELLO"
    
    FastAPI-->>Client: 200 OK
    Note over FastAPI,Client: { "upper": "HELLO" }
```

### 6. リストの合計値（POST /sum-list）
...
sequenceDiagram
    ...
    Client->>FastAPI: POST /sum-list
    Note over Client,FastAPI: { "numbers": [1, 2, 3, 4, 5] }
    
    FastAPI->>Validator: リクエストボディ検証
    Validator->>Validator: list[int]型チェック
    ...
    alt 検証エラー
        ...
        Validator-->>FastAPI: 検証OK
        FastAPI->>API: sum_list(numbers)
        API->>Logic: sum(numbers) 実行
        Logic-->>API: 15
        API-->>FastAPI: 15
        FastAPI-->>Client: 200 OK
        Note over FastAPI,Client: { "sum": 15 }
    end
```
...
sequenceDiagram
    ...
    Client->>FastAPI: POST /concat
    Note over Client,FastAPI: { "a": "Hello", "b": "World" }
    
    FastAPI->>API: concat_strings(a, b)
    API->>Logic: a + b 実行
    Logic-->>API: "HelloWorld"
    API-->>FastAPI: "HelloWorld"
    
    FastAPI-->>Client: 200 OK
    Note over FastAPI,Client: { "concat": "HelloWorld" }
```
...
| 2026-02-13 | repository-AのGET /api/usersフロー追加、repository-Bの3つの新規エンドポイントフロー追加（/upper, /sum-list, /concat） | Claude Sonnet 4.5 |
