...
sequenceDiagram
    ...
    participant API_Module as api.py (fetch_data)

    Client->>FastAPI: GET /hello
    FastAPI->>API_Module: fetch_data()
    API_Module-->>FastAPI: {"result": "success", "data": [1, 2, 3]}
    FastAPI-->>Client: 200 OK
    Note over FastAPI,Client: { "message": "HELLO", "api_data": {...} }
```
...
sequenceDiagram
    ...
    participant API_Module as api.py (process_data)
    ...
    alt 検証エラー
        ...
        FastAPI->>API_Module: process_data([5, 3])
        API_Module-->>FastAPI: [10, 6]
        FastAPI->>Logic: a + b 計算
        Logic-->>FastAPI: 結果: 8
        FastAPI-->>Client: 200 OK
        Note over FastAPI,Client: { "result": 8, "processed": [10, 6] }
    end
```
...
| 2026-02-13 | repository-AにGET /api/usersフロー追加、repository-Bのフローにapi.py連携を追加 | Claude Sonnet 4 |
...
**最終更新日時（日本時間）**: 2026年2月13日 10:50:36
